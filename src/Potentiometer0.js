...
class Potentiometer0 extends React.Component {
    constructor(props) {
        super(props)
    ...
    componentDidMount() {
        this.setState({
            ...
            settingIsOpen: false,
            p0Interval: store.get('p0Interval', 1000),
            value: "Not Started",
    ...
    streamData() {
        const url = `http://${this.state.ipAddress}/p0`
        ...
        axios.get(url, {timeout: 2000}).then(response => {
            ...
            if(numberOfErrors > 10) {
                ...
                if(this.state.alertFlag) {
                    ipcRenderer.send('connection-error', 'Potentiometer 0')
    ...
    startStream() {
        this.setState({
            stream: setInterval(this.streamData, this.state.p0Interval),
            isReading: true,
    ...
    onIntervalSettingSubmit(e) {
        ...
        let p0Interval = e.target.interval.value

        store.set('p0Interval', p0Interval)
        
        this.setState({
            p0Interval: p0Interval,
            settingIsOpen: false,
            settingButtonLabel: "Show Setting",
            buttonSettingClass: "btn-outline-dark"
        })
                        ...
                        <form id="windowSizeSetting" onSubmit={this.onIntervalSettingSubmit}>
                            ...
                            <div className="form-row">
                                <div className="form-group col-md-12">
                                    ...
                                    <input className="form-control form-control-lg" type="text" name="interval" placeholder={this.state.p0Interval} />
                                </div>
